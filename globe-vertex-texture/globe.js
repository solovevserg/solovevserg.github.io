...
DAT.Globe = function (container, colorFn) {
  ...
  function modelLoader() {
    ...
    loader.load({
      model: pointModel, callback: function (g) {
        pointGeo = g;
        gridLoader()
      }
    });
  ...
  function gridLoader() {
    loader = new THREE.JSONLoader();
    loader.load({
      model: gridModel, callback: function (g) {
        gridGeo = g;
        init();
        createPoints();
      }
  ...
  function rotateIntervally(event) {
    // mouse.x = - event.clientX;
    // mouse.y = event.clientY;

    var zoomDamp = distance / 1000;

    target.x = target.x + 5 * 0.005 * zoomDamp;
    // target.y = target.y + 2 * 0.005 * zoomDamp;
