...
### Вариант B — Только CSS-медиазапрос, без JS *(отклонён)*
...
### Вариант C — `@nuxtjs/color-mode` *(выбранный)*
...
:root { --bg: #0c0c0e; ... }
:root[data-theme="light"] { --bg: #f5f5f3; ... }

// станет: обе темы явно через атрибут
:root[data-theme="dark"]  { --bg: #0c0c0e; ... }
:root[data-theme="light"] { --bg: #f5f5f3; ... }
```

Одновременно следует добавить CSS-медиазапрос как базовый fallback на случай, если JS не отработал:
```less
@media (prefers-color-scheme: dark)  { :root { --bg: #0c0c0e; ... } }
@media (prefers-color-scheme: light) { :root { --bg: #f5f5f3; ... } }
```
